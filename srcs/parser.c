...
static int	parseLine(char *line, t_data **Data, int fd)
{
	...
	if (line[i]){
		if (!ft_strncmp(&line[i], "NO ", 3))
			return (parse_texture(&line[i+2], &(*Data)->MapDisplay->textures->North));
		else if (!ft_strncmp(&line[i], "SO  ", 3))
			return (parse_texture(&line[i+2], &(*Data)->MapDisplay->textures->South));
		else if (!ft_strncmp(&line[i], "WE ", 3))
			return (parse_texture(&line[i+2], &(*Data)->MapDisplay->textures->West));
		else if (!ft_strncmp(&line[i], "EA ", 3))
			return (parse_texture(&line[i+2], &(*Data)->MapDisplay->textures->East));
